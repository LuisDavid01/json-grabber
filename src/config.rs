...
impl TryFrom<Vec<String>> for Operation {
    ...
    fn try_from(value: Vec<String>) -> Result<Self> {
        ...
        if term == "add" {
            if value.len() != 3 {
                return Err(anyhow!(
                    "add expected 2 arguments but got {} ",
                    value.len() - 1
                ));
            }
        ...
        if term == "rm" {
            if value.len() != 2 {
                return Err(anyhow!(
                    "remove expected 2 arguments but got {} ",
                    value.len() - 1
                ));
            }
        ...
        if value.len() > 1 {
            return Err(anyhow!(
                "Print expected 1 argument but got {} ",
                value.len()
            ));
        }
        let arg = value.pop().expect("to exist");
...
fn get_pwd(pwd: Option<PathBuf>) -> Result<PathBuf> {
    ...
    let loc = std::env::current_dir().context("unable to get current dir")?;
    return Ok(loc);
}
...
mod test {
    ...
    fn test_add_key() -> Result<()> {
        ...
        assert_eq!(
            opts.operation,
            Operation::Add(String::from("foo"), String::from("var"),)
        );
